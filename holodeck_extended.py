...
def generate_single_scene(args):
    ...
    generated = args.model.generate_scene(
        scene=scene,
        query=args.query,
...
def add_object(scene, model, llm_client):
    ...
    for oo_str in object_list:
        ...
        if (pos == "floor"):
            ...
            visualize_asset(candidates[0][0], model.objaverse_asset_dir)
            visualize_asset(candidates[1][0], model.objaverse_asset_dir)
            visualize_asset(candidates[2][0], model.objaverse_asset_dir)

            user_selection = input("Which of the following objects you'd like to add? Choose A, B, or C: ")
            if user_selection == "A":
                selected_candidate = candidates[0]
            elif user_selection == "B":
                selected_candidate = candidates[1]
            elif user_selection == "C":
                selected_candidate = candidates[2]
            ...
            room_layer = parse_room_layer(scene, model, room_type)
            ...
            for obj_name, a_id in add_candidates:
                ...
                existing = parse_existing_floor(scene, model, room_type)
                object_information = ""
